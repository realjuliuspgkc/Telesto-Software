...
log "S2 Guidance Begin at: " + _CLOCK_TIME(missionTime) to _EVT_DATA_DIRECTORY.
    ...
    until ship:apoapsis >= _PARK_ORBIT - 2500 and ship:periapsis > body:atm:height - 2000 {
        ...
        // Throttle & steering apparently?
            _INCLINE_MANAGER(5).
            lock steering to heading(_Heading_Control, _Pitch_Control, -90). // Steer the damn rocket.
...
LOCAL FUNCTION _ORBITAL_INSERTION { // S2 Orbital Insertion
    ...
    until eta:apoapsis <= 1 {
        if ship:periapsis < _PARK_ORBIT - 150 {set maxthrot to 0.3.}
        else if ship:periapsis < _PARK_ORBIT - 100 {set maxthrot to 0.2.}
